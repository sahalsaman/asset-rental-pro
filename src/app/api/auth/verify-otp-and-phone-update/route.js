...
export async function POST(req) {
  ...
  if (!user.onboardingCompleted) {
    await UserModel.findByIdAndUpdate(user._id, { onboardingCompleted: true, phone: newPhone, countryCode: newCountryCode })
  } else {
    await UserModel.findByIdAndUpdate(user._id, { phone: newPhone, countryCode: newCountryCode })
  }
  const token = setTokenValue(user);
