...
import { Edit, Pencil, PlusIcon, ReceiptIndianRupee, Trash } from "lucide-react";
...
export default function UnitDetailPage() {
  ...
  useEffect(() => {
    if (!unitId || !current_property?._id) router.push(`/business/units`);
    fetchUnit()
    fetchBookings();
  ...
  if (!unit) return <div className="p-8 md:px-16 px-5"><DashboardSkeleton /></div>;

  const breadcrumbItems = [
    { label: "Home", href: "/business" },
    { label: "Units", href: `/business/units/${unitId}` },
  ...
  return (
    <div >
      {/* Unit Header */}
      <div className="flex flex-col justify-between items-start  gap-6 mb-6 bg-slate-50 p-8 pt-5 shadow-sm border md:px-16 px-5">
      ...
      <div className=" md:px-16 px-5 ">
        <div className="flex justify-between  items-center  mb-5">
          ...
          {unit.status === UnitStatus.AVAILABLE || unit.status === UnitStatus.PARTIALLY_BOOKED ? <Button onClick={() => setShowBookingModal(true)} variant="green" className="whitespace-nowrap">
            <PlusIcon className="w-6 h-6" /> Add Booking
          </Button> : ""}
        </div>
